...
function App() {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (isDisplayMenuOpen && displayMenuRef.current && !displayMenuRef.current.contains(event.target as Node)) {
        setIsDisplayMenuOpen(false);
      }

      // Handle bookmark modal
      if ((isAddingBookmark || isEditingBookmark) && bookmarkModalRef.current && !bookmarkModalRef.current.contains(event.target as Node)) {
      ...
      if (isManagingCategories && categoryModalRef.current && !categoryModalRef.current.contains(event.target as Node)) {
        setIsManagingCategories(false);
        setNewCategory("");
      }

      // Handle category visibility menu
      if (isCategoryVisibilityMenuOpen && categoryVisibilityMenuRef.current && !categoryVisibilityMenuRef.current.contains(event.target as Node)) {
  ...
  const deleteCategory = (categoryId: string) => {
    ...
    const updatedBookmarks = bookmarks.map((bookmark) => (bookmark.categoryId === categoryId ? { ...bookmark, categoryId: "default" } : bookmark));
    saveBookmarks(updatedBookmarks);
  ...
  const toggleCategoryVisibility = (categoryId: string) => {
    const updatedHiddenCategories = hiddenCategories.includes(categoryId) ? hiddenCategories.filter((id) => id !== categoryId) : [...hiddenCategories, categoryId];

    saveHiddenCategories(updatedHiddenCategories);
  };

  const filteredBookmarks = bookmarks.filter((bookmark) => {
    const matchesSearch = bookmark.title.toLowerCase().includes(searchTerm.toLowerCase()) || bookmark.description.toLowerCase().includes(searchTerm.toLowerCase()) || bookmark.tags.some((tag) => tag.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const getBookmarksContainerClass = () => {
    return displaySettings.categoryLayout === "horizontal" ? "flex overflow-x-auto pb-4 space-x-4 scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900" : "space-y-4";
  };
  ...
  const exportBookmarksAsJson = () => {
    ...
    a.href = url;
    a.download = "netflix-bookmarks.json";
  ...
  const categoriesToDisplay = categories.filter((category) => !hiddenCategories.includes(category.id)).filter((category) => selectedCategory === "all" || category.id === selectedCategory);
    ...
    <div className="min-h-screen bg-[#141414] text-white p-8">
      <div className="max-w-7xl mx-auto">
        <header className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Search bookmarks..." className="bg-gray-800 rounded-full py-2 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-red-600" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            {/* Add display settings button */}
            <div className="relative">
              <button onClick={() => setIsDisplayMenuOpen(!isDisplayMenuOpen)} className="bg-gray-800 hover:bg-gray-700 rounded-full p-2" title="Display settings">
                <Settings className="w-6 h-6" />
              </button>

              {/* Display settings dropdown */}
              {isDisplayMenuOpen && (
                <div ref={displayMenuRef} className="absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-gray-800 ring-1 ring-black ring-opacity-5 z-50">
                  <div className="p-4 space-y-4">
                    <div>
                      <h3 className="text-sm font-medium mb-2">Card Size</h3>
                      <div className="flex gap-2">
                        {(["small", "medium", "large"] as const).map((size) => (
                          <button key={size} onClick={() => saveDisplaySettings({ ...displaySettings, cardSize: size })} className={`px-3 py-1 rounded-md text-sm ${displaySettings.cardSize === size ? "bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => saveDisplaySettings({ ...displaySettings, categoryLayout: "grid" })} className={`px-3 py-1 rounded-md text-sm flex items-center gap-1 ${displaySettings.categoryLayout === "grid" ? "bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}>
                          <LayoutGrid className="w-4 h-4" />
                          Grid
                        </button>
                        <button onClick={() => saveDisplaySettings({ ...displaySettings, categoryLayout: "flex" })} className={`px-3 py-1 rounded-md text-sm flex items-center gap-1 ${displaySettings.categoryLayout === "flex" ? "bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}>
                          <Folder className="w-4 h-4" />
                          Flex
                        </button>
                        {/* Add new horizontal layout option */}
                        <button onClick={() => saveDisplaySettings({ ...displaySettings, categoryLayout: "horizontal" })} className={`px-3 py-1 rounded-md text-sm flex items-center gap-1 ${displaySettings.categoryLayout === "horizontal" ? "bg-red-600" : "bg-gray-700 hover:bg-gray-600"}`}>
            ...
            <div className="relative">
              <button onClick={() => setIsCategoryVisibilityMenuOpen(!isCategoryVisibilityMenuOpen)} className="bg-gray-800 hover:bg-gray-700 rounded-full p-2" title="Toggle category visibility">
              ...
              {isCategoryVisibilityMenuOpen && (
                <div ref={categoryVisibilityMenuRef} className="absolute right-0 mt-2 w-64 rounded-md shadow-lg bg-gray-800 ring-1 ring-black ring-opacity-5 z-50">
                  <div className="p-4 space-y-2">
                    <h3 className="text-sm font-medium mb-2">Show/Hide Categories</h3>
                    {categories.map((category) => (
                      <div key={category.id} className="flex items-center">
                        <input type="checkbox" id={`visibility-${category.id}`} checked={!hiddenCategories.includes(category.id)} onChange={() => toggleCategoryVisibility(category.id)} className="rounded text-red-600 focus:ring-red-500 h-4 w-4 mr-2" />
            ...
            <button onClick={() => setIsAddingBookmark(true)} className="bg-red-600 hover:bg-red-700 rounded-full p-2" title="Add new bookmark">
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          <button onClick={() => setSelectedCategory("all")} className={`px-4 py-2 rounded-full ${selectedCategory === "all" ? "bg-red-600" : "bg-gray-800 hover:bg-gray-700"}`}>
            All
          </button>
          {categories.map((category) => (
            <button key={category.id} onClick={() => setSelectedCategory(category.id)} className={`px-4 py-2 rounded-full ${selectedCategory === category.id ? "bg-red-600" : "bg-gray-800 hover:bg-gray-700"}`}>
              {category.name}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {getAllTags().map((tag) => (
            <button key={tag} onClick={() => toggleTag(tag)} className={`px-3 py-1 rounded-full text-sm flex items-center gap-1 ${selectedTags.includes(tag) ? "bg-red-600" : "bg-gray-800 hover:bg-gray-700"}`}>
        ...
        <DragDropContext onDragEnd={handleDragEnd}>
          <div className={getCategoryLayoutClass()}>
            {categoriesToDisplay.map((category) => (
              <Droppable key={category.id} droppableId={category.id} direction={displaySettings.categoryLayout === "horizontal" ? "horizontal" : "vertical"}>
                {(provided) => (
                  <div className="space-y-4">
                    ...
                    <div ref={provided.innerRef} {...provided.droppableProps} className={getBookmarksContainerClass()} style={displaySettings.categoryLayout === "horizontal" ? { minHeight: "100px" } : {}}>
                      {filteredBookmarks
                          ...
                          <Draggable key={bookmark.id} draggableId={bookmark.id} index={index}>
                            {(provided) => (
                              <div
                                ...
                                className={`bg-gray-800 ${getCardSizeClass()} ${getCardSizeHeight()} rounded-lg overflow-hidden hover:scale-105 transition-transform cursor-pointer ${displaySettings.categoryLayout === "horizontal" ? "flex-shrink-0" : ""}`}
                                onClick={(e) => {
                                ...
                                <div className="p-4">
                                  <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                                      <button onClick={() => editBookmark(bookmark)} className="text-gray-400 hover:text-blue-500">
                                        <Edit2 className="w-5 h-5" />
                                      </button>
                                      <button onClick={() => deleteBookmark(bookmark.id)} className="text-gray-400 hover:text-red-600">
                                        <X className="w-5 h-5" />
                                      </button>
                                    </div>
                                  </div>
                                  ...
                                  <div className="flex flex-wrap gap-2 mt-3">
                                    {bookmark.tags.map((tag, i) => (
                                      <span key={i} className="bg-gray-700 text-xs px-2 py-1 rounded-full flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                                        <Tag className="w-3 h-3" />
                                        {tag}
                                      </span>
                                    ))}
        ...
        {(isAddingBookmark || isEditingBookmark) && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div ref={bookmarkModalRef} className="bg-gray-800 rounded-lg p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">{isEditingBookmark ? "Edit Bookmark" : "Add New Bookmark"}</h2>
                <button
                  onClick={() => {
                    setIsAddingBookmark(false);
                    setIsEditingBookmark(false);
              ...
              <div className="space-y-4">
                <input type="text" placeholder="Title" className="w-full bg-gray-700 rounded p-2" value={newBookmark.title} onChange={(e) => setNewBookmark({ ...newBookmark, title: e.target.value })} />
                <div className="flex gap-2">
                  <input type="url" placeholder="URL" className="flex-1 bg-gray-700 rounded p-2" value={newBookmark.url} onChange={(e) => setNewBookmark({ ...newBookmark, url: e.target.value })} />
                  <button onClick={() => newBookmark.url && fetchOgImage(newBookmark.url)} className="bg-blue-600 hover:bg-blue-700 rounded px-3 py-2" disabled={!newBookmark.url || isLoadingImage}>
                    <Image className="w-5 h-5" />
                  </button>
                </div>
                <input type="url" placeholder="Custom Image URL (optional)" className="w-full bg-gray-700 rounded p-2" value={newBookmark.imageUrl} onChange={(e) => setNewBookmark({ ...newBookmark, imageUrl: e.target.value })} />
                <textarea placeholder="Description" className="w-full bg-gray-700 rounded p-2" value={newBookmark.description} onChange={(e) => setNewBookmark({ ...newBookmark, description: e.target.value })} />
                ...
                <div className="space-y-2">
                  <div className="flex gap-2">
                    ...
                    />
                    <button onClick={addTagToBookmark} className="bg-blue-600 hover:bg-blue-700 rounded px-3 py-2">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {newBookmark.tags.map((tag, i) => (
                          <div key={i} className="bg-red-600 text-white text-sm px-2 py-1 rounded-full flex items-center gap-1">
                            <Tag className="w-3 h-3" />
                            <span>{tag}</span>
                            <button onClick={() => removeTagFromBookmark(tag)} className="ml-1 hover:bg-red-700 rounded-full p-1">
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                        ))}
                ...
                <select className="w-full bg-gray-700 rounded p-2" value={newBookmark.categoryId} onChange={(e) => setNewBookmark({ ...newBookmark, categoryId: e.target.value })}>
                  {categories.map((category) => (
                ...
                <button onClick={isEditingBookmark ? updateBookmark : addBookmark} className="w-full bg-red-600 hover:bg-red-700 rounded p-2 font-semibold">
        ...
        {isManagingCategories && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div ref={categoryModalRef} className="bg-gray-800 rounded-lg p-6 w-full max-w-md">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">Manage Categories</h2>
                <button onClick={() => setIsManagingCategories(false)} className="text-gray-400 hover:text-red-600">
                  <X className="w-6 h-6" />
                </button>
              </div>
              <div className="space-y-4">
                <div className="flex gap-2">
                  <input type="text" placeholder="New Category Name" className="flex-1 bg-gray-700 rounded p-2" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} />
                  <button onClick={addCategory} className="bg-red-600 hover:bg-red-700 rounded px-4 py-2 font-semibold">
                    Add
                  </button>
                </div>
                <div className="space-y-2">
